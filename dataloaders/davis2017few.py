...
def fewShot(paired_sample, n_ways, n_shots, cnt_query, davis=True):
    ...
    support_mask = [[getMask(support_labels[0][shot], support_num_objs[way], support_num_objs)
    ...
    for i, query_label_tmp in enumerate(query_labels_tmp):
        ...
        for j in range(len(support_num_objs)):
            query_label_tmp[label_t[i]==support_num_objs[j]] = j  # it doesn't have to be j + 1
    ...
    return {'class_ids': class_ids,
            ...
            'support_mask': support_mask,    # length of support mask is not way,we have to modify many code
