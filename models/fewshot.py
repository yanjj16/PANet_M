...
# pre_mask 
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs , pre_mask):
        """
        Args:
        ...
        for epi in range(batch_size):
            ...
            fg_dist = [sum(self.calDist(qry_fts[:, epi], prototype))/obj_nums for prototype in fg_prototypes]
            bg_dist = self.calDist(qry_fts[:, epi], bg_prototype)
            dist = [bg_dist,]+fg_dist
            all_pred = torch.stack(dist*16, dim=1)  # N x 16(1 + Wa) x H' x W'
            ...
            outputs.append(self.decoder(r5, all_pred, r4, r3, r2, pre_mask_concat[[epi]]))
