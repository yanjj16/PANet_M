...
class davis2017(BaseDataset):
    ...
    def get_Imgids(self,class_id):   
        ...
        for _, _, img_id in os.walk(image_dir):
            for way in img_id:
                ...
                way = way.strip('\n')
                img_ids.append(way)
        return img_ids   # img_ids is the name list of a certa
    ...
    def P2msks(self, Img, obj_ids):
        ...
        for idx in obj_ids:
            ...
            Imgs.append(torch.Tensor(np.array(label)).long())
        return Imgs

    def msks2P(self, msks, obj_ids):
        # if max_num == 1:
        #     return msks[0]
        ...
        for idx, msk in enumerate(msks):
            ...
            if len(ids) > 0:
                P[ids[:, 0], ids[:, 1], ids[:, 2]] = idx   # distinguish different objects by idx+1
        return P
    ...
    def __getitem__(self, item):  
        ...
        semantic_mask = self.P2msks(semantic_mask,obj_ids)
        semantic_mask = self.msks2P(semantic_mask,obj_ids)
        ...
        pre_semantic_mask = self.P2msks(pre_semantic_mask, obj_ids)
        pre_semantic_mask = self.msks2P(pre_semantic_mask, obj_ids)
